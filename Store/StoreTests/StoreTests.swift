...
"""
    ...
    func testDifferentItem() {
        let itemPrice = 2_200_00
        let itemName = "Laptop"
        let item = Item(name: itemName, priceEach: itemPrice)
        XCTAssertEqual(2_200_00, item.priceEach)
        register.scan(item)
        XCTAssertEqual(2_200_00, register.subtotal())
    }
}
